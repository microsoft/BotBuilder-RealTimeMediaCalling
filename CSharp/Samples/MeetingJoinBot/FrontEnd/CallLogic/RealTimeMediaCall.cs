...
namespace FrontEnd.CallLogic
{
    ...
    internal class RealTimeMediaCall : IRealTimeMediaCall
    {
        ...
        private Task OnJoinCallAppHostedMediaCompleted(JoinCallAppHostedMediaOutcomeEvent outgoingJoinCallOutcomeEvent)
        {
            ...
            try
            {
                if (outgoingJoinCallOutcomeEvent.JoinCallAppHostedMediaOutcome.Outcome == Outcome.Success)
                {
                    Log.Info(new CallerInfo(), LogContext.FrontEnd, $"[{CallId}] Join Call is successfull");
                    outgoingJoinCallOutcomeEvent.RealTimeMediaWorkflow.NotificationSubscriptions =
                        new NotificationType[] { NotificationType.CallStateChange, NotificationType.RosterUpdate };


                }
                else
                {
                    Log.Info(new CallerInfo(), LogContext.FrontEnd,
                        $"[{CallId}] Join Call is failed, failure reason: ");
                    //cleanup internal resources
                    MediaSession.Dispose();
                    CallService.EndCall();
                }
